...
pk')$ and reveal $pk'$. Let someone pick a number between $0$ and $n - 1$ (for
$n \leq q$, to be exact) and compute the linear combination $pk'' = pk' \cdot
pk^n \pmod{p}$. You then reveal $sk'' := sk' + n \cdot sk \pmod{q}$ and your
challenger checks that $g^{sk''} = pk'' \pmod{p}$. This protocol does the same
(and some more) than the one above with $n$ envelopes. To see why, consider the
point of view of the challenger who knows $pk, pk'$ and $pk''$ and has just
picked $n$. Unless you know the correct $sk$, or are able to compute such a
$sk$, there is only one single value of $n$ for which you have any hope of
providing the correct answer that will convince your challenger. Your
probability of cheating is bounded by $1/n$ and $n$ can ba chosen as large as
you like (up to $q-1$).

Suppose that there are two distinct values of $n$, namely $n_1$ and $n_2$, for
...
\begin{eqnarray}
g^{sk_1} & = & pk' \cdot pk^{n_1} \pmod{p} \\
g^{sk_2} & = & pk' \cdot pk^{n_2} \pmod{p}
\end{eqnarray}
which suggests that we divide the two, cancelling $pk'$:
\begin{eqnarray}
g^{sk_1} / g^{sk_2} & = & pk^{n_1} / pk^{n_2} \pmod{p} \\
g^{sk_1 - sk_2 \pmod{q}} & = & pk^{n_1 - n_2 \pmod{q}} \pmod{p}
\end{eqnarray}
but the exponent space is a field and $n_1, n_2$ are distinct so we can
rearrange to get
\begin{eqnarray}
g^{\frac{sk_1 - sk_2}{n_1 - n_2}\pmod{q}} & = & pk \pmod{p}
\end{eqnarray}
...
challenges $n_1, n_2$ then from this information you can compute a secret key
$sk = \frac{sk_1 - sk_2}{n_1 - n_2}\pmod{q}$ yourself.
