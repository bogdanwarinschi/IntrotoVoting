...
The verification algorithm is deterministic and takes a verification key,
message and signature as input and outputs 0 or 1. We call a signature $\sigma$
valid for message $m$ and key $vk$ if $\alg{Verify}(vk, m, \sigma) = 1$.
...
with in cryptographic proofs. We follow the presentation of Fujioka et al. \cite{FOO92} since we are working towards their voting protocol; a blind signature scheme that has an explicit form is for example the one of Chaum \cite{C85}.
...
work of Bernhard \cite{B14} and we develop the Schnorr \cite{S??},
Chaum-Pedersen \cite{CP??} and disjunctive Chaum-Pedersen (DCP) protocols.
