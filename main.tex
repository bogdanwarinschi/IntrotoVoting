...
\newcommand{\cryptosubsection}[1]{\subsection[#1]{
\raisebox{-3pt}{\includegraphics[height=14pt]{lock}}
#1}}
\newcommand{\cryptosubsubsection}[1]{\subsubsection[#1]{
\raisebox{-3pt}{\includegraphics[height=14pt]{lock}}
#1}}
...
%We briefly summarize the content of these notes and indicate 
...
In particular, we show how to realize all of the primitives that go into the design of the Helios voting systems.   This is one of the few voting systems that are actually used in practice and serves as a  example that illustrates many (if not all) of the concepts relevant to electronic voting.   
Just like for the higher-level constructions we avoid giving proofs. 
...
Sections that introduce basic cryptographic primitives are marked with a lock symbol (\raisebox{-3pt}{\includegraphics[height=14pt]{lock}}). Sections giving applications specific to the Helios voting
scheme are marked with a \Sun\ symbol. 
%Sections introducing cryptographic concepts are marked with a padlock symbol.
...
\cryptosubsection{Digital signatures}
...
\cryptosubsubsection{One-time signatures.}
...
some signature scheme generates, for each action that she performs in the
...
\cryptosubsection{Blind signatures}
...
A blind signature scheme is correct if the following outputs $v = 1$ for any
...
key $(N, e)$. The analysis for this scheme is due to Bellare et al. and \cite{BNPS01} and we omit it in this work. 

\cryptosubsection{Commitment schemes}
...
\cryptosubsection{Asymmetric encryption}
...
\cryptosubsection{Homomorphic encryption}
...
\cryptosubsection{Threshold encryption}
...
\cryptosubsection{Zero-knowledge proofs}
...
\footnotetext{There is a model for verifiability by Kuesters et al.
\cite{KTV11} but it coniders voting schemes from a highly abstract point of
view and has, to our knowledge, never been successfully applied to a
fully cryptographic scheme such as Helios.}
